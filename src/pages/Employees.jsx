...
import { Avatar, Badge, Modal, FormField, ConfirmDialog, LoadingState } from '../components/ui'
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          const fullName = employee.name || [employee.firstName, employee.lastName].filter(Boolean).join(' ').trim()

          return (
            <div className="flex items-center gap-3">
              <Avatar name={fullName} src={employee.avatar} size="sm" />
              <div>
                <p className="text-sm font-semibold text-slate-800">{fullName || 'Employee'}</p>
...
function EmployeeModal({ isOpen, onClose, onSave, initial, saving }) {
  ...
  useEffect(() => {
    ...
    setForm({
      name: initial.name || [initial.firstName, initial.lastName].filter(Boolean).join(' ').trim(),
      email: initial.email || '',
      phone: initial.phone || '',
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Join Date">
            <input type="date" className="input-field" value={form.joinDate} onChange={set('joinDate')} />
          </FormField>
        </div>
        <div className="grid grid-cols-2 gap-4">
